...
Ora che conosciamo l'architettura del web, iniziamo a scrivere codice. Per questo laboratorio, useremo \textbf{Visual Studio Code (VS Code)} come nostro strumento principale.

\section{Esercizi Guidati}
Il materiale per questi esercizi si trova nella cartella \texttt{src/lezione1/} del repository.

\subsection{Esercizio 1: Il browser come interprete}
...
\textbf{Task}: Cambia il colore di sfondo della pagina scrivendo \texttt{document.body.style.backgroundColor = 'yellow';} nello script.

\subsection{Esercizio 2: Manipolazione del DOM}
Il \textbf{DOM (Document Object Model)} è l'interfaccia che permette a JavaScript di agire sulla pagina. Ogni tag HTML diventa un oggetto JavaScript. \texttt{getElementById} è il metodo base per cercare un nodo specifico nell'albero.
\begin{lstlisting}[language=HTML]
<div id="container">
    <p id="first">Primo</p>
</div>
...
\end{lstlisting}
\textbf{Task}: Modifica il testo del paragrafo \texttt{first} usando \texttt{document.getElementById('first').innerText = 'Testo modificato';}.

\subsection{Esercizio 3: Gestione degli Eventi}
Il browser è un ambiente \textit{event-driven}. Un evento è un segnale di notifica. \texttt{addEventListener} associa una funzione (callback) a un tipo di evento (click, mouseover, keydown).
\begin{lstlisting}[language=HTML]
...
\end{lstlisting}
\textbf{Task}: Aggiungi un ascoltatore per l'evento \texttt{mouseover} sul bottone che cambia il colore del testo.

\subsection{Esercizio 4: Form Modulare}
...
\end{lstlisting}
L'evento \texttt{submit} va intercettato chiamando \texttt{e.preventDefault()} per evitare che il browser ricarichi la pagina.
\textbf{Task}: Nel file \texttt{index.js}, aggiungi un campo \texttt{number} per l'età nel form e mostralo nell'alert al submit.

\subsection{Esercizio 5: Gestione delle liste}
Possiamo creare nodi HTML dinamicamente. \texttt{createElement} crea un nuovo oggetto DOM non ancora visibile, \texttt{appendChild} lo inserisce come ultimo figlio di un genitore.
\begin{lstlisting}[language=HTML]
...
\end{lstlisting}
\textbf{Task}: Aggiungi un bottone "Rimuovi" che elimina l'ultimo elemento della lista usando \texttt{lista.lastElementChild.remove();}.
