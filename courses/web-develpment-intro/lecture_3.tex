...
\section{Comunicazione Real-time: Webhooks}
Mentre le API tradizionali seguono un modello \textbf{Pull} (il client richiede dati quando ne ha bisogno), i \textbf{Webhooks} implementano un modello \textbf{Push}. Un Webhook è essenzialmente una "Reverse API": è il server che invia una notifica al client quando accade un evento specifico.

\subsection{Perch\'e sono necessari?}
Senza i Webhooks, un client dovrebbe interrogare continuamente il server per verificare se ci sono nuovi dati (\textit{Polling}). Questo approccio è inefficiente per due motivi:
\begin{enumerate}
    \item \textbf{Spreco di Risorse}: La maggior parte delle richieste restituirà "nessun nuovo dato", consumando CPU e banda inutilmente.
    \item \textbf{Latenza}: Se il client interroga il server ogni 10 minuti, riceverà il dato con un ritardo medio di 5 minuti. I Webhooks permettono una reazione quasi istantanea.
\end{enumerate}
...
\begin{tikzpicture}[
    ...
    % Polling
    \node (c1) [box, fill=gray!5] {Client};
    \node (s1) [box, fill=primary!10, right=4cm of c1] {Server};
    \draw [arrow] ([yshift=0.3cm]c1.east) -- node[above, scale=0.6] {1. C'è qualcosa?} ([yshift=0.3cm]s1.west);
    \draw [arrow] ([yshift=0.1cm]s1.west) -- node[below, scale=0.6] {2. No} ([yshift=0.1cm]c1.east);
    \draw [arrow] ([yshift=-0.2cm]c1.east) -- node[above, scale=0.6] {3. C'è qualcosa?} ([yshift=-0.2cm]s1.west);
    \draw [arrow] ([yshift=-0.4cm]s1.west) -- node[below, scale=0.6] {4. Sì (+Dati)} ([yshift=-0.4cm]c1.east);
    \node at ($(c1.south)!0.5!(s1.south)+(0,-0.5cm)$) [font=\bfseries\small] {Polling (Inaccurate)};

    % Webhook
    \begin{scope}[yshift=-3cm]
        \node (c2) [box, fill=gray!5] {Client\\(Listener)};
        \node (s2) [box, fill=primary!10, right=4cm of c2] {Server\\(Emitter)};
        \draw [arrow] (s2.west) -- node[above, scale=0.7] {Evento Accaduto (HTTP POST)} (c2.east);
        \node at ($(c2.south)!0.5!(s2.south)+(0,-0.5cm)$) [font=\bfseries\small] {Webhook (Real-time)};
    \end{scope}
\end{tikzpicture}
\caption{Confronto tra Polling (Pull) e Webhook (Push).}
\label{fig:polling_vs_webhook}
\end{figure}

\subsection{Funzionamento Tecnico}
Per utilizzare un Webhook, il client deve esporre un \textbf{Endpoint pubblico} (una URL) e "registrarlo" presso il servizio del server. Quando l'evento si verifica (es. ricezione di un pagamento, nuovo messaggio in chat), il server effettua una richiesta HTTP (solitamente \texttt{POST}) verso quell'indirizzo, inviando il payload JSON dell'evento.

\subsection{Sviluppo Locale e Tunnelling: ngrok}
Durante lo sviluppo, il vostro server (es. un'applicazione Flask o Express) viene eseguito su \texttt{localhost}. Tuttavia, i server esterni (come GitHub, Stripe o Webex) non possono raggiungere il vostro computer locale poich\'e protetto da firewall e sprovvisto di un indirizzo IP pubblico raggiungibile.

Per risolvere questo problema si utilizza il \textbf{Tunnelling} tramite strumenti come \textbf{ngrok}.
\begin{itemize}
    \item \textbf{Cosa fa}: Crea un tunnel sicuro tra un URL pubblico (es. \texttt{https://xyz.ngrok-free.app}) e la vostra porta locale (es. \texttt{5000}).
    \item \textbf{Utilità}: Permette ai server esterni di inviare richieste POST direttamente al vostro ambiente di sviluppo locale, rendendo possibile il test dei Webhooks in tempo reale.
\end{itemize}
...
\begin{tikzpicture}[
    box/.style={rectangle, draw, rounded corners, minimum width=2.5cm, minimum height=1cm, align=center, font=\small},
    cloud/.style={draw, ellipse, fill=gray!10, minimum width=2.5cm, minimum height=1cm, align=center, font=\small},
    arrow/.style={-latex, line width=1pt}
]
    \node (webex) [box, fill=primary!20] {Webex Server\\(Emitter)};
    \node (ngrok_c) [cloud, right=2.5cm of webex] {ngrok Cloud\\(Tunnel)};
    \node (ngrok_l) [box, fill=gray!5, right=2.5cm of ngrok_c] {ngrok Agent\\(Local)};
    \node (flask) [box, fill=primary!10, below=1cm of ngrok_l] {Flask App\\(Localhost:5000)};

    \draw [arrow] (webex) -- node[above, scale=0.7] {Public POST URL} (ngrok_c);
    \draw [arrow, dashed] (ngrok_c) -- node[above, scale=0.7] {Secure Tunnel} (ngrok_l);
    \draw [arrow] (ngrok_l) -- node[right, scale=0.7] {Local Forward} (flask);
\end{tikzpicture}
\caption{Funzionamento di ngrok per il test locale di Webhooks.}
\label{fig:ngrok_tunnel}
\end{figure}
...
\begin{itemize}[leftmargin=3em]
    ...
    \item \textbf{Sorting}: Definizione dell'ordine dei risultati (es. \texttt{GET /orders?sort=created\_at:desc}).
\end{itemize}
