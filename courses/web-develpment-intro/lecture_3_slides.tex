...
\begin{frame}{Monoliti vs Microservizi}
    ...
    \begin{tikzpicture}[
        ...
        \draw [arrow] (gw.east) -- (s1.west);
        \draw [arrow] (gw.east) -- (s2.west);
        \draw [arrow] (gw.east) -- (s3.west);
        \draw [arrow] (gw.east) -- (s4.west);
    \end{tikzpicture}
    }
    \vspace{0.3cm}
    \begin{columns}
...
\begin{frame}{Comunicazione Real-time: Webhooks}
    \begin{itemize}
        \item \textbf{Polling (Pull)}: Il client chiede continuamente "Ci sono novità?".
        \item \textbf{Webhook (Push)}: Il server chiama il client quando succede qualcosa.
    \end{itemize}
    \centering
    \resizebox{0.7\textwidth}{!}{
    \begin{tikzpicture}[
        ...
        \draw [arrow] ([yshift=0.2cm]c1.east) -- (s1.west);
        \draw [arrow] (s1.west) -- (c1.east);
        \node at ($(c1.south)!0.5!(s1.south)+(0,-0.3cm)$) [font=\scriptsize] {Polling (Pull) - Inefficiente};

        % Webhook
        \begin{scope}[yshift=-2cm]
            \node (c2) [box, fill=gray!5] {Client (URL)};
            \node (s2) [box, fill=primary!10, right=4cm of c2] {Server};
            \draw [arrow, primary] (s2.west) -- node[above, black, scale=0.7] {HTTP POST} (c2.east);
            \node at ($(c2.south)!0.5!(s2.south)+(0,-0.3cm)$) [font=\scriptsize] {Webhook (Push) - Real-time};
        \end{scope}
    \end{tikzpicture}
    }
\end{frame}

\begin{frame}{Testare i Webhooks: ngrok}
    \begin{itemize}
        \item \textbf{Problema}: Il vostro PC (\textit{localhost}) non è raggiungibile dall'esterno.
        \item \textbf{Soluzione}: Un tunnel sicuro verso il mondo esterno.
    \end{itemize}
    \centering
    \resizebox{0.8\textwidth}{!}{
    \begin{tikzpicture}[
        box/.style={rectangle, draw, rounded corners, minimum width=2.5cm, minimum height=0.8cm, align=center, font=\scriptsize},
        cloud/.style={draw, ellipse, fill=gray!10, minimum width=2.5cm, minimum height=0.8cm, align=center, font=\scriptsize},
        arrow/.style={-latex, line width=1pt}
    ]
        \node (webex) [box, fill=primary!20] {Webex Server};
        \node (ngrok) [cloud, right=2cm of webex] {ngrok Cloud};
        \node (local) [box, fill=gray!5, right=2cm of ngrok] {ngrok Agent};
        \node (flask) [box, fill=primary!10, below=0.8cm of local] {Flask (Port 5000)};

        \draw [arrow] (webex) -- (ngrok);
        \draw [arrow, dashed] (ngrok) -- (local);
        \draw [arrow] (local) -- (flask);
    \end{tikzpicture}
    }
    \vspace{0.3cm}
    \begin{block}{Comando ngrok}
        \texttt{ngrok http 5000} $\rightarrow$ Fornisce un URL \textbf{https} pubblico.
    \end{block}
\end{frame}
...
\begin{frame}[fragile]{Autenticazione Bearer Token}
    \begin{itemize}
        \item REST deve essere \textbf{stateless}.
        \item Si evitano i Cookie/Sessioni.
        \item Si invia un token nell'header HTTP \texttt{Authorization}.
    \end{itemize}
    \vspace{0.5cm}
    \begin{block}{Header HTTP}
...
\begin{frame}{Versioning, Filtering e Sorting}
    \begin{itemize}
        \item \textbf{Versioning}: Evitare \textit{breaking changes}.
        \begin{itemize}
            \item URI: \texttt{/api/v1/resource} (standard).
            \item Header: \texttt{X-API-Version: 2}.
        \end{itemize}
        \item \textbf{Filtering}: Restringere i risultati tramite Query Params.
        \begin{itemize}
            \item Esempio: \texttt{/users?role=admin\&status=active}.
        \end{itemize}
        \item \textbf{Sorting}: Definire l'ordine dei dati.
        \begin{itemize}
            \item Esempio: \texttt{/orders?sort=date:desc}.
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Laboratorio Avanzato: Bot e Gruppi}
    \begin{block}{Task 1: Gestione Stanze}
        \begin{itemize}
            \item Crea una stanza: \texttt{POST /v1/rooms}.
            \item Salva l'ID restituito.
        \end{itemize}
    \end{block}
    \begin{block}{Task 2: Estendi il Bot}
        Partendo da \texttt{bot.py} (Flask + ngrok):
        \begin{itemize}
            \item Aggiungi il comando \texttt{/info}.
            \item Rispondi con il nome della stanza e numero membri.
            \item Endpoint membri: \texttt{GET /v1/memberships}.
        \end{itemize}
    \end{block}
\end{frame}
