...
\end{lstlisting}
\section{Esercizio 5: Gestione Dispositivi di Rete (REST API)}
L'obiettivo di questo esercizio è progettare e implementare un'API REST per la gestione di un inventario di dispositivi di rete (Router).

\subsection{Modellazione della Risorsa}
Ogni dispositivo (Router) deve essere rappresentato da un oggetto JSON con la seguente struttura:
\begin{itemize}
    \item \texttt{id}: Identificativo univoco (intero).
    \item \texttt{hostname}: Nome del dispositivo (stringa).
    \item \texttt{interfaces}: Un array di oggetti interfaccia, ognuno contenente:
    \begin{itemize}
        \item \texttt{name}: Nome dell'interfaccia (es. "GigabitEthernet0").
        \item \texttt{ip}: Indirizzo IPv4.
        \item \texttt{netmask}: Maschera di rete.
        \item \texttt{description}: Descrizione testuale dello scopo dell'interfaccia.
    \end{itemize}
\end{itemize}

\subsection{Task 1: Implementazione CRUD}
Implementate un server (in Express o Flask) che esponga i seguenti endpoint per la risorsa \texttt{/routers}:
\begin{itemize}
    \item \textbf{Lettura Totale} (\texttt{GET /routers}): Restituisce la lista di tutti i router.
    \item \textbf{Creazione} (\texttt{POST /routers}): Aggiunge un nuovo router all'inventario.
    \item \textbf{Aggiornamento} (\texttt{PUT /routers/:id}): Modifica i dati di un router esistente.
    \item \textbf{Cancellazione} (\texttt{DELETE /routers/:id}): Rimuove un router dall'inventario.
\end{itemize}

\subsection{Task 2: Generazione Configurazione}
Aggiungete un endpoint speciale \textbf{\texttt{GET /routers/:id/config}} che, invece di restituire JSON, restituisca un payload in formato \textbf{testo semplice} (\texttt{text/plain}).

L'endpoint deve generare una stringa che simuli una configurazione CLI (stile Cisco) basata sui dati salvati. Ad esempio:
\begin{lstlisting}[language=bash, caption=Esempio di output atteso per /config]
...
\textbf{Suggerimento}: Ciclate sull'array \texttt{interfaces} del router trovato e concatenate le stringhe per costruire il corpo della risposta.
