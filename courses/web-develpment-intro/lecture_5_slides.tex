...
\begin{frame}[fragile]{Setup e Installazione}
    Il modo più rapido per iniziare è usare l'applicazione ufficiale di generazione.
    \begin{block}{Comando di Setup}
        \texttt{npx create-next-app@latest my-app}
    \end{block}
    \textbf{Opzioni Consigliate}:
    \begin{itemize}
        \item \textbf{App Router}: SI (il nuovo standard).
        \item \textbf{Tailwind CSS}: SI (stili pronti all'uso).
        \item \textbf{src/ directory}: SI (organizzazione pulita).
    \end{itemize}
    \vspace{0.3cm}
    \textbf{Avvio}: \texttt{npm run dev} $\rightarrow$ \texttt{localhost:3000}
\end{frame}

\begin{frame}{Struttura del Progetto}
    \begin{itemize}
        \item \textbf{\texttt{app/}}: Il cuore del routing. Ogni file \texttt{page.js} è una pagina.
        \item \textbf{\texttt{layout.js}}: Definisce la struttura comune (HTML, Body).
        \item \textbf{\texttt{globals.css}}: Dove vive Tailwind.
        \item \textbf{\texttt{public/}}: Immagini e asset statici.
    \end{itemize}
    \centering
    \begin{tikzpicture}[
        node/.style={rectangle, draw, rounded corners, fill=blue!5, font=\tiny, minimum width=2cm}
    ]
        \node (app) [node] {app/};
        \node (page) [node, below left=0.5cm and -0.5cm of app] {page.js};
        \node (lay) [node, below right=0.5cm and -0.5cm of app] {layout.js};
        \node (api) [node, below=1.2cm of app] {api/ (Backend)};
...
\begin{frame}[fragile]{Case Study: Network API}
    A differenza dei task, un Router ha una struttura annidata. Dobbiamo mappare array dentro altri oggetti.
    \begin{lstlisting}[language=javascript]
// Esempio di mapping annidato
{routers.map(router => (
  <div key={router.id} className="card">
    <h3>{router.hostname}</h3>
    <ul>
      {router.interfaces.map(iface => (
        <li key={iface.name}>{iface.ip}</li>
      ))}
    </ul>
  </div>
))}
    \end{lstlisting}
    \textbf{Best Practice}: Usare il \textit{Optional Chaining} (\texttt{?.}) per evitare errori se i dati non sono ancora stati caricati.
\end{frame}
